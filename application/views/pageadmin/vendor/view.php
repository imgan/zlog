<section class="content">
				...
				<form class="form-horizontal" role="form" id="formTambah">
					<div class="card card-info">
						<div class="modal-header">
							<h4 class="modal-title">Add Jenis Layanan Vendor</h4>
						...
						<div class="card-body">
							<div class="input-group mb-3">
								...
								<input required type="text" id="nama" name="nama" class="form-control" placeholder="Jenis Layanan Vendor">
							</div>
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				<form class="form-horizontal" role="form" id="formEdit">
					<div class="card card-info">
						<div class="modal-header">
							<h4 class="modal-title">Edit Layanan Vendor</h4>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
						</div>
						<div class="card-body">
							<div class="input-group mb-3">
								...
								<input required type="text" id="e_nama" name="e_nama" class="form-control" placeholder="Jenis Layanan Vendor">
							</div>
	...
	<div class="card">
		<div class="card-header">
			<h3 class="card-title">Daftar Layanan Vendor</h3>
		</div>
		<br>
		<div class="col-sm-2">
			<button href="#modalTambah" type="button" role="button" data-toggle="modal"
			 class="btn btn-block btn-primary"><a class="ace-icon fa fa-plus bigger-120"></a> Add Layanan Vendor</button>
		</div>
		<br>
...
<script type="text/javascript">
	if ($("#formTambah").length > 0) {
		$("#formTambah").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					url: "<?php echo base_url('administrator/vendor/simpan') ?>",
					type: "POST",
	...
	$('#show_data').on('click', '.item_hapus', function() {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('administrator/vendor/delete') ?>",
					async: true,
					dataType: "JSON",
					data: {
						id: id,
	...
	function show_data() {
		$.ajax({
			type: 'POST',
			url: '<?php echo site_url('administrator/vendor/tampil') ?>',
			async: true,
			dataType: 'json',
			success: function(data) {
				var html = '';
	...
	$('#show_data').on('click', '.item_edit', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('administrator/vendor/tampil_byid') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
	...
	if ($("#formEdit").length > 0) {
        $("#formEdit").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: "<?php echo base_url('administrator/vendor/update') ?>",
                    type: "POST",
